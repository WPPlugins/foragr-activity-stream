<?php
define('FGR_USER_AGENT', 'Foragr-WPPlugin/1.0');
define('FGR_SOCKET_TIMEOUT', 10);

function fgr_get_query_string($postdata) {
	$postdata_str = '';
...
function fgr_get_post_content($boundary, $postdata, $file_name, $file_field) {
	if(!$file_name || !$file_field) {
		return fgr_get_query_string($postdata);
	}
...
function fgr_get_http_headers_for_request($boundary, $content, $file_name, $file_field) {
	$headers = array();
	$headers[] = 'User-Agent: ' . FGR_USER_AGENT;
...
function _fgr_curl_urlopen($url, $postdata, &$response, $file_name, $file_field) {
	$c = curl_init($url);
	$postdata_str = fgr_get_query_string($postdata);

	$c_options = array(
		CURLOPT_USERAGENT		=> FGR_USER_AGENT,
		CURLOPT_RETURNTRANSFER	=> true,
		CURLOPT_POST			=> ($postdata_str ? 1 : 0),
		CURLOPT_HTTPHEADER		=> array('Expect:'),
		CURLOPT_TIMEOUT 		=> FGR_SOCKET_TIMEOUT
	);
	if($postdata) {
...
function _fgr_fsockopen_urlopen($url, $postdata, &$response, $file_name, $file_field) {
	$buf = '';
	$req = '';
	$length = 0;
	$boundary = '----------' . md5(time());
	$postdata_str = fgr_get_post_content($boundary, $postdata, $file_name, $file_field);
	...
	$fp = @fsockopen($url_pieces['host'], $url_pieces['port'], $errno, $errstr, FGR_SOCKET_TIMEOUT);
	if(!$fp) { return false; }
	...
	$req .= 'Host: ' . $host . "\r\n";
	$req .=  fgr_get_http_headers_for_request($boundary, $postdata_str, $file_name, $file_field);
	if($postdata_str) {
		$req .= "\r\n\r\n" . $postdata_str;
	}
	$req .= "\r\n\r\n";
...
function _fgr_fopen_urlopen($url, $postdata, &$response, $file_name, $file_field) {
	$params = array();
	if($file_name && $file_field) {
		$boundary = '----------' . md5(time());
		$content = fgr_get_post_content($boundary, $postdata, $file_name, $file_field);
		$header = fgr_get_http_headers_for_request($boundary, $content, $file_name, $file_field);

		$params = array('http' => array(
			'method'	=> 'POST',
			'header'	=> $header,
			'content'	=> $content,
			'timeout'	=> FGR_SOCKET_TIMEOUT
		));
	} else {
		if($postdata) {
			$params = array('http' => array(
				...
				'content'	=> fgr_get_query_string($postdata),
				'timeout'	=> FGR_SOCKET_TIMEOUT
			));
		}
	}
	...
	ini_set('user_agent', FGR_USER_AGENT);
 ...
 */
function fgr_urlopen($url, $postdata=false, $file=false) {
	$response = array(
		'data' => '',
		'code' => 0
	);
...
//
	...
	if(function_exists('curl_init')) {
		...
		_fgr_curl_urlopen($url, $postdata, $response, $file_name, $file_field);
	} else if(ini_get('allow_url_fopen') && function_exists('stream_get_contents')) {
		_fgr_fopen_urlopen($url, $postdata, $response, $file_name, $file_field);
	} else {
		// TODO: Find the failure condition for fsockopen() (sockets?)
		_fgr_fsockopen_urlopen($url, $postdata, $response, $file_name, $file_field);
	}
